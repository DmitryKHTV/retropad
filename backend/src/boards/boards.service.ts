...
import {
  BoardAccessService,
  type EffectiveRole,
} from '../board-access/board-access.service';
...
export type ColumnWithVotes = Omit<RawColumn, 'stickers'> & {
...
export class BoardsService {
  ...
  async findOne(
    id: string,
    requesterId: string,
  ): Promise<BoardWithColumnsAndRole> {
    ...
    const totalBySticker = new Map(
      totals.map((row) => [row.stickerId, row._sum?.count ?? 0]),
    );
    const mineBySticker = new Map(
      myVotes.map((vote) => [vote.stickerId, vote.count]),
    );
