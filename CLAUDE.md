...
**Deployed 2026-09-07 — the stack is live on the VPS** at `retropad.dkhomutov.dev` / `api-retropad.dkhomutov.dev`, built on the server (`git pull && docker compose -f docker-compose.prod.yml up -d --build`) and verified by hand.

One bug surfaced on the first build there, and it is exactly the kind local verification cannot catch:

- **`COPY --from=build /app/public ./public` failed with `stat app/public: file does not exist`.** `frontend/public/` existed on the dev machine but was **empty**, and git versions files, not directories — so the folder never reached the repository. A local `docker build` reads the working copy and stayed green; the server builds from a fresh `git clone`, where that path simply does not exist. Fixed with `frontend/public/.gitkeep` (added with `git add -f` — the folder matched a `.gitignore` rule). The general lesson: "what is on my disk" and "what is in git" are two different build contexts, and only building from a clean clone tells them apart — one more argument for CI.

Still to do before real users: rate limiting on `/auth` (`@nestjs/throttler`), `set_real_ip_from` for Cloudflare so client IPs are neither wrong nor forgeable, `helmet` + the cheap security headers, and error/loading states on the frontend (`BoardPage` renders `null` forever when the board query fails). Operationally still open: the cron line for `scripts/backup-db.sh` is not installed on the server, ufw is `inactive` (deliberately — it is a footgun on this host, see above), and `DEPLOY.md` still stops after "Clone and configure". Then GitHub Actions CI.
